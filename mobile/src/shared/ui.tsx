...
export function Screen({ children }: PropsWithChildren) {
  ...
  const styles = StyleSheet.create({
    ...
    inner: { flex: 1, paddingHorizontal: 18, paddingTop: 10, paddingBottom: 16, gap: 14 },
  });
  return (
...
export function SectionCard({ children }: PropsWithChildren) {
  ...
  return <Card style={styles.card} mode="contained">{children}</Card>;
}

export function ScorePill({ label, value }: { label: string; value: string }) {
  const colors = useAppColors();
  const styles = StyleSheet.create({
    pill: { backgroundColor: colors.surfaceAlt, borderRadius: 16, paddingHorizontal: 12, paddingVertical: 8, gap: 1, minWidth: 88 },
...
export function MetricBar({ label, value }: { label: string; value: number }) {
  ...
  return (
    <View style={styles.metricWrap}>
      ...
      <ProgressBar progress={safe / 100} color={colors.primary} style={styles.progress} />
    </View>
  );
}
