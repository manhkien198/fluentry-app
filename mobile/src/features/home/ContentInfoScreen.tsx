...
export function ContentInfoScreen() {
  ...
  const [version, setVersion] = useState<{ version?: string; checksum?: string; updated_at?: string }>({});
    ...
    <Screen>
      <SectionCard>
        <Card.Content style={s.block}>
          ...
          <Text style={s.line}>{t('content.version')}:  {version.version || t('content.na')}</Text>
          <Text style={s.line}>{t('content.checksum')}:  {version.checksum || t('content.na')}</Text>
          <Text style={s.line}>{t('content.updated')}:  {version.updated_at || t('content.na')}</Text>
        </Card.Content>
      </SectionCard>
    </Screen>
  );
