    ...
    <Screen>
      <SectionCard>
        <Card.Content style={s.block}>
          ...
          <Text style={s.line}>
            {t('trends.today_minutes', { today: data.today_minutes ?? 0, target: data.daily_target_minutes ?? 15 })}
          </Text>
          {loading ? (
            <>
      ...
      <SectionCard>
        <Card.Content style={s.block}>
          ...
          <Text style={s.line}>
            {t('trends.overall_values', { value: (data.trend?.overall || []).slice(-5).join(', ') || t('common.na') })}
          </Text>
          <Text style={s.line}>
            {t('trends.pronunciation_values', { value: (data.trend?.pronunciation || []).slice(-5).join(', ') || t('common.na') })}
          </Text>
          <Text style={s.line}>
            {t('trends.fluency_values', { value: (data.trend?.fluency || []).slice(-5).join(', ') || t('common.na') })}
          </Text>
        </Card.Content>
      </SectionCard>
      ...
      <SectionCard>
        <Card.Content style={s.block}>
          ...
          {(data.achievements || []).map((a) => (
            <Text key={a.id} style={s.line}>• {a.title}: {a.value}</Text>
          ))}
          {!(data.achievements || []).length ? <Text style={s.line}>{t('trends.no_achievements')}</Text> : null}
        </Card.Content>
      </SectionCard>
    </Screen>
  );
