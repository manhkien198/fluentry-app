...
export function HistoryScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((item) => item.session_id.toLowerCase().includes(q) || (item.lesson_title || '').toLowerCase().includes(q));
  }, [items, query]);
  ...
  return (
    <Screen>
      ...
      {loadError ? <Text style={[s.empty, { color: colors.warning }]}>{loadError}</Text> : null}
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={s.empty}>{query.trim() ? t('history.no_match') : t('history.empty')}</Text>}
        renderItem={({ item }) => (
          <PressScale>
            <Card mode="contained" style={s.card}>
            <Card.Content style={s.row}>
              <View style={{ flex: 1, paddingRight: 10 }}>
                <Text style={s.lesson}>{item.lesson_title || t('history.lesson_fallback')}</Text>
