...
def test_send_password_reset_email_sends(monkeypatch: pytest.MonkeyPatch):
  class FakeSMTP:
    def __init__(self, host: str, port: int, timeout: int):
      self.started_tls = False
      self.logged_in = False
  ...
  mailer.send_password_reset_email("a@example.com", "token")
...
def test_send_password_reset_email_requires_host(monkeypatch: pytest.MonkeyPatch):
  monkeypatch.setattr(mailer, "SMTP_HOST", "")
