...
def test_practice_line_109_hit(monkeypatch):
  class User:
    id = "u1"
...
def test_session_store_no_user_filter_branch(monkeypatch):
  class Row:
    session_id = "s"
    user_id = "u"
    lesson_id = "l"
  ...
  out = session_store.list_sessions(user_id=None)
  assert len(out) == 1


def test_user_metrics_warning_only_branch():
  weak = user_metrics._extract_weak_sounds_from_result({"phonemes": [], "words": [{"text": "abc", "status": "warning"}]})
  assert weak == ["abc"]


def test_scorer_branch_26_and_edges(monkeypatch):
  monkeypatch.setattr(
    scorer,
    ...
    lambda *_a, **_k: {
      "alignment": {
        "segments": [{"text": "..."}],
        "phones": [{"text": "AA", "word": "x", "start_ms": 0, "end_ms": 1}],
        "alignment_status": "mfa",
        "engine_meta": {},
      },
      "phoneme_map": [],
      "features": {"audio_penalty": 50, "energy": 0.1, "rhythm": 0.1, "word_count": 1, "estimated_duration_ms": 1},
    },
  )
...
def test_textgrid_branches_precise(tmp_path):
